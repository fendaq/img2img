...
import math
import time
from model_generator import create_generator_deepunet, create_generator_pix2pix
...
def create_model(inputs, targets):
    ...
    with tf.variable_scope("generator"):
        ...
        outputs = create_generator_deepunet(inputs, out_channels, a.ngf, training=a.mode=='train')
...
def get_data_generator():
    while True:
        inputs = np.random.randn(a.batch_size, a.crop_size, a.crop_size, 3)
        targets = np.random.randn(a.batch_size, a.crop_size, a.crop_size, 3)
...
def main():
    ...
    print("examples count = %d" % len(glob.glob(os.path.join(a.input_dir, 'A', '*.*'))))

    inputs = tf.placeholder(tf.float32, [a.batch_size, a.crop_size, a.crop_size, 3], name='inputs')
    targets = tf.placeholder(tf.float32, [a.batch_size, a.crop_size, a.crop_size, 3], name='targets')
        ...
        if a.mode == "test":
            ...
            for step in range(max_steps):
                ...
                if should(a.progress_freq):
                    ...
                    print("progress  step %d  image/sec %0.1f  remaining %dm" % (train_step, rate, remaining / 60))
